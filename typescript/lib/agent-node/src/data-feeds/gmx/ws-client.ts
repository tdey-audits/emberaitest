...
export interface GMXOracleUpdate {
  token: string;
  price: string;
  timestamp: number;
  blockNumber: number;
...
export class GMXWSClient implements FeedClient {
  ...
  private handleMessage(data: string): void {
    try {
      ...
      if (message.type === 'price') {
        this.handlePriceUpdate(message as unknown as GMXPriceUpdate);
      } else if (message.type === 'oracle') {
        this.handleOracleUpdate(message as unknown as GMXOracleUpdate);
      }
    } catch (error) {
  ...
  private handlePriceUpdate(update: GMXPriceUpdate): void {
    const minPrice = parseFloat(update.minPrice);
    const maxPrice = parseFloat(update.maxPrice);
  ...
  private handleOracleUpdate(update: GMXOracleUpdate): void {
    const oracleData: OracleData = {
      feedId: update.token,
      value: BigInt(update.price),
      timestamp: update.timestamp,
      decimals: update.oracleDecimals,
      source: 'gmx',
    };
